...
def drawTicTacToeBoard(x_offset: int = 0, y_offset: int = 0) -> None:
    ...
    cross_lookup = [
        [
            [
                x_offset+cross_padding, # v
                y_offset+cross_padding, # v
                x_offset+round(board_size*midpoint_offset[0])-cross_padding, # v
                y_offset+round(board_size*midpoint_offset[0])-cross_padding-line_thickness  # v
            ],
            [
                x_offset+cross_padding, # v
                y_offset+round(board_size*midpoint_offset[0])-cross_padding, # v
                x_offset+round(board_size*midpoint_offset[0])-cross_padding, # v
                y_offset+cross_padding, # x
            ]
        ],
    ]
    circle_lookup = [
